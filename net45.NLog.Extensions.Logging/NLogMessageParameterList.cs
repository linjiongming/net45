...
namespace NLog.Extensions.Logging
{
    ...
    {
        private static readonly NLogMessageParameterList EmptyList = new NLogMessageParameterList(new KeyValuePair<string, object>[] {}, null, false, false);
        private static readonly NLogMessageParameterList PositionalParameterList = new NLogMessageParameterList(new KeyValuePair<string, object>[] {}, null, hasComplexParameters: false, isPositional: true);
        private static readonly NLogMessageParameterList PositionalComplexParameterList = new NLogMessageParameterList(new KeyValuePair<string, object>[] {}, null, hasComplexParameters: true, isPositional: true);
        private static readonly NLogMessageParameterList OriginalMessageList = new NLogMessageParameterList(new[] { new KeyValuePair<string, object>(NLogLogger.OriginalFormatPropertyName, string.Empty) }, 0, false, false);
        ...
        public static NLogMessageParameterList TryParse(IReadOnlyList<KeyValuePair<string, object>> parameterList)
        {
            ...
            {
                if (IsValidParameterList(parameterList, out int? originalMessageIndex, out bool hasComplexParameters, out bool isPositional))
                {
                    if (isPositional)
                    {
        ...
        {
            int parameterCount = parameterList.Count;
            List<KeyValuePair<string, object>> validParameterList = new List<KeyValuePair<string, object>>(parameterCount);

            for (int i = 0; i < parameterCount; ++i)
            {
                if (!TryGetParameterName(parameterList, i, out string parameterName))
                    continue;
        ...
        public MessageTemplateParameter this[int index]
        {
            get
            {
                KeyValuePair<string, object> parameter = _parameterList[index < _originalMessageIndex ? index : index + 1];
        ...
        {
            CaptureType capture = GetCaptureType(parameterName[0]);
